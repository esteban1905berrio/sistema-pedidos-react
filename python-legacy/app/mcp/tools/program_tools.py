...
def register_program_tools(mcp: FastMCP, service_getter: Callable[[], ProgramService]):
    """
    Register program-related tools with the MCP server.

    Args:
        mcp: FastMCP server instance
        program_service: Callable that returns ProgramService instance (lazy-loaded) for ABAP program operations
    """
    ...
    def get_program_source(
        ...
        """
        return service_getter().get_program_source(program_name, version=version)
    ...
    def get_include_source(
        ...
        """
        return service_getter().get_include_source(program_name, include_name, version=version)
