...
FUNCTION Z_CX_GET_TRANSPORT_INFO
  IMPORTING
    VALUE(IV_TRANSPORT_NUMBERS) TYPE STRING
  EXPORTING
  ...
  DATA: lt_transports TYPE STANDARD TABLE OF ty_transport,
        ...
        lv_json       TYPE string,
        lt_trkorr     TYPE STANDARD TABLE OF trkorr,
        lr_trkorr     TYPE RANGE OF trkorr,

        lv_trkorr     TYPE trkorr,
        lv_first      TYPE abap_bool VALUE abap_true.

  CLEAR: ev_success, ev_message, ev_transports_json.

  " Validate input
  IF iv_transport_numbers IS INITIAL.
    ev_success = ''.
    ev_message = 'Transport number(s) required'.
    RETURN.
  ENDIF.

  " Split comma-separated list
  CONDENSE iv_transport_numbers NO-GAPS.
  SPLIT iv_transport_numbers AT ',' INTO TABLE lt_trkorr.

  lr_trkorr = VALUE #( FOR es_trkorr IN lt_trkorr WHERE ( table_line IS NOT INITIAL )
                        ( sign = 'I' option = 'EQ'  low = es_trkorr ) ).

  CHECK lr_trkorr  IS NOT INITIAL.

  " Query E070 with JOIN to E071 and tasks in single query (ABAP 7.5)
  SELECT e070~trkorr,
         e070~trfunction,
         e070~trstatus,
         e070~as4user,
         ...
         e070~strkorr,
         e07t~as4text,
         COUNT( DISTINCT e071~trkorr ) AS object_count,
    ...
    WHERE e070~trkorr IN @lr_trkorr
  ...
  IF sy-subrc <> 0.
    ev_success = ''.
    ev_message = |No transports found for: { iv_transport_numbers }|.
    RETURN.
  ENDIF.
  ...
  LOOP AT lt_transports INTO ls_transport.
    ...
    " Map types and statuses to descriptions
    DATA(lv_type_desc) = COND string(
      WHEN ls_transport-trfunction = 'K' THEN 'Workbench'
      WHEN ls_transport-trfunction = 'S' THEN 'Task'
      WHEN ls_transport-trfunction = 'T' THEN 'Transport of Copies'
      WHEN ls_transport-trfunction = 'W' THEN 'Workbench Request'
