 ...
 * 9. analyze_abap_deeply - Deep analysis of complex ABAP objects (using
 * extraction)
 *
 * Usage via MCP:
...
public class SapPromptProvider {
         ...
         */
        @McpPrompt(name = "review_abap_code", title = "ABAP Code Review", description = "Reviews ABAP code following Crystal development standards. Analyzes quality, performance, security, and best practices.")
         ...
         */
        @McpPrompt(name = "analyze_transport", title = "Transport Analysis", description = "Analyzes SAP transport request for risks, dependencies, and release recommendations.")
         ...
         */
        @McpPrompt(name = "explain_class", title = "Class Explanation", description = "Explains ABAP class purpose, architecture, methods, and usage patterns.")
         ...
         */
        @McpPrompt(name = "debug_dump", title = "ABAP Dump Analysis", description = "Analyzes ABAP short dumps (ST22) and provides root cause analysis with fix suggestions.")
         ...
         */
        @McpPrompt(name = "migration_checklist", title = "Migration Checklist", description = "Generates comprehensive pre-release/migration checklist for SAP deployments.")
         ...
         */
        @McpPrompt(name = "generate_unit_test", title = "Generate Unit Test", description = "Generates ABAP Unit test class with test methods, mocks, and assertions.")
         ...
         */
        @McpPrompt(name = "document_function_module", title = "Document Function Module", description = "Generates comprehensive documentation for ABAP function module with usage examples.")
        ...
        // ========================================================================
        // 8. ANALYZE_ABAP_DEEPLY - Deep Analysis Prompt (Extraction-based)
        // ========================================================================

        /**
         * Conducts a deep analysis of complex ABAP objects (Programs, Function Groups).
         *
         * This prompt instructs the agent to PRIORITIZE extraction over simple reading
         * to ensure full context (includes, global variables) is available.
         *
         * Workflow:
         * 1. Extract object to local filesystem (extract_abap_components)
         * 2. Read local files (Main + Includes)
         * 3. Analyze comprehensive context
         *
         * @param objectName   Object name (e.g. ZREP_TEST, ZFG_TEST)
         * @param objectType   Object type (PROG, FUGR, CLAS, etc.)
         * @param analysisGoal Goal of the analysis (Refactoring, Bug fix,
         *                     Understanding)
         * @return Structured deep analysis prompt
         */
        @McpPrompt(name = "analyze_abap_deeply", title = "Deep ABAP Analysis", description = "Analyzes complex ABAP objects by enforcing extraction first. Essential for Function Groups and Programs with includes to avoid context loss.")
        public GetPromptResult analyzeAbapDeeply(
                        @McpArg(name = "objectName", description = "Object name to analyze (e.g. ZREP_TEST, ZFG_TEST)", required = true) String objectName,
                        @McpArg(name = "objectType", description = "Object type (PROG, FUGR)", required = true) String objectType,
                        @McpArg(name = "analysisGoal", description = "Goal of analysis (e.g. 'Refactor logic', 'Find global var usage')", required = true) String analysisGoal) {
                log.info("Prompt request: analyze_abap_deeply for {}", objectName);

                String promptText = """
                                # Deep ABAP Analysis Request

                                You are an expert SAP Architect tasked with a deep analysis of a complex ABAP object.

                                **Object**: %s (%s)
                                **Goal**: %s

                                ## CRITICAL INSTRUCTION: DO NOT USE `get_object_source`

                                This object is likely complex (Function Group or Program with Includes). using `get_object_source` or `get_program_source` is INSUFFICIENT because it fails to load global data (TOP includes), subroutines (FORM includes), or shared dependencies.

                                ## REQUIRED WORKFLOW

                                1.  **EXTRACT FIRST**: Use the tool `extract_abap_components` (or `extract_abap_objects` if available) to download the ENTIRE structure of `%s` to the local filesystem.
                                2.  **READ FILES**: Use `list_dir` to see the extracted structure, and `view_file` to read the Main program AND the relevant Includes (TOP, Forms).
                                3.  **ANALYZE**: Once you have the full files in your context, proceed with the analysis goal: %s.

                                ## Why this is necessary?
                                - **Global Variables**: Defined in TOP include, invisible to single-source tools.
                                - **Forms/Subroutines**: Often in separate includes F01, F02...
                                - **Modules**: PBO/PAI logic often distributed.

                                Please acknowledge this plan and start by calling the extraction tool.
                                """
                                .formatted(objectName, objectType, analysisGoal, objectName, analysisGoal);

                return new GetPromptResult(
                                "Deep Analysis: " + objectName,
                                List.of(new PromptMessage(Role.USER, new TextContent(promptText))));
        }

        // ========================================================================
        // 9. COMPARE_VERSIONS - Version Comparison Prompt
        // ========================================================================
         ...
         */
        @McpPrompt(name = "compare_versions", title = "Compare Versions", description = "Compares active vs inactive versions of ABAP objects and analyzes changes.")
         ...
         */
        @McpPrompt(name = "analyze_package_for_extraction", title = "Analyze Package for Extraction", description = "Analyzes SAP package contents and recommends which objects to extract for training data, reference, or audit purposes.")
         ...
         */
        @McpPrompt(name = "generate_extraction_report", title = "Generate Extraction Report", description = "Generates comprehensive report for ABAP code extraction with statistics, quality metrics, and recommendations.")
         ...
         */
        @McpPrompt(name = "explain_cds_view", title = "Explain CDS View", description = "Explains CDS View structure, associations, annotations, and usage patterns in S/4HANA.")
         ...
         */
        @McpPrompt(name = "explain_enhancement", title = "Explain Enhancement/BAdI", description = "Explains SAP enhancement implementations (BAdI, Enhancement Spot, CMOD, BTE) with business logic and testing guidance.")
         ...
         */
        @McpPrompt(name = "evaluate_code_quality_batch", title = "Batch Code Quality Evaluation", description = "Evaluates code quality for multiple ABAP objects, providing quality scores and recommendations for extraction filtering.")
