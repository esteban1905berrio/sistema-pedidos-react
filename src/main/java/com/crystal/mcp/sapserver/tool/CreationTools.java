...
public class CreationTools {
         ...
         * @param packageName       package (e.g., "$TMP" for local, "ZPACKAGE" for
         *                          development)
         * @param transport         optional transport number (null for local objects in
         *                          $TMP)
        ...
        public CreationResult create_function_group(
                        @McpToolParam(description = "Name of the function group. " +
                                        ...
                                        "Examples: 'ZTEST_FG', 'ZFI_UTILS', 'ZMMI_PROCESS'", required = true) String functionGroupName,
                        @McpToolParam(description = "Description of the function group (max 60 chars). " +
                                        "Example: 'Test Function Group for Invoice Processing'", required = true) String description,
                        @McpToolParam(description = "Package name. " +
                                        "Use '$TMP' for local objects (no transport). " +
                                        "Use development package for shared objects (requires transport). " +
                                        "Examples: '$TMP', 'ZPACKAGE', 'ZFI'", required = true) String packageName,
                        @McpToolParam(description = "Optional transport number. " +
                                        "Required for non-local packages. " +
                                        "Leave null for $TMP objects. " +
                                        "Examples: 'CADK911122', 'DEVK900123', null", required = false) String transport) {
         ...
         * <p>
         * Function modules are reusable procedures that can be called from ABAP
         * programs.
        ...
        public CreationResult create_function_module(
                        @McpToolParam(description = "Name of the function module. " +
                                        ...
                                        "Examples: 'Z_TEST_FM', 'Z_GET_INVOICE', 'Z_PROCESS_ORDER'", required = true) String functionModuleName,
                        @McpToolParam(description = "Parent function group name (must exist). " +
                                        "Examples: 'ZTEST_FG', 'ZFI_UTILS', 'ZMMI_PROCESS'", required = true) String functionGroupName,
                        @McpToolParam(description = "Description of the function module (max 60 chars). " +
                                        "Example: 'Get Invoice Details by Number'", required = true) String description,
                        @McpToolParam(description = "Optional transport number. " +
                                        "Required for non-local function groups. " +
                                        "Leave null for $TMP objects. " +
                                        "Examples: 'CADK911122', 'DEVK900123', null", required = false) String transport,
                        @McpToolParam(description = "Processing type of the function module. " +
                                        ...
                                        +
                                        "Examples: null (normal FM), 'rfc' (RFC-enabled FM)", required = false) String processingType) {
                return creationService.createFunctionModule(functionModuleName, functionGroupName, description,
                                transport, processingType);
        }
        ...
        public CreationResult create_class(
                        @McpToolParam(description = "Name of the class. " +
                                        ...
                                        "Examples: 'ZCL_TEST', 'ZCL_INVOICE_PROCESSOR', 'YCL_UTILS'", required = true) String className,
                        @McpToolParam(description = "Description of the class (max 60 chars). " +
                                        "Example: 'Invoice Processor Class'", required = true) String description,
                        @McpToolParam(description = "Package name. " +
                                        "Use '$TMP' for local objects (no transport). " +
                                        "Use development package for shared objects (requires transport). " +
                                        "Examples: '$TMP', 'ZPACKAGE', 'ZFI'", required = true) String packageName,
                        @McpToolParam(description = "Optional transport number. " +
                                        ...
                                        "Leave null for $TMP objects. " +
                                        "Examples: 'CADK911122', 'DEVK900123', null", required = false) String transport,
                        @McpToolParam(description = "Optional superclass name (for inheritance). " +
                                        "Leave null for classes without inheritance. " +
                                        "Examples: 'CL_ABAP_CHAR_UTILITIES', 'ZCL_BASE_PROCESSOR', null", required = false) String superclass) {
        ...
        public CreationResult create_interface(
                        @McpToolParam(description = "Name of the interface. " +
                                        "Must start with letter, max 30 chars, only A-Z0-9_. " +
                                        "Convention: Start with ZIF_ or YIF_ for custom interfaces. " +
                                        "Examples: 'ZIF_TEST', 'ZIF_PROCESSOR', 'YIF_LOGGER'", required = true) String interfaceName,
                        @McpToolParam(description = "Description of the interface (max 60 chars). " +
                                        "Example: 'Processor Interface'", required = true) String description,
                        @McpToolParam(description = "Package name. " +
                                        "Use '$TMP' for local objects (no transport). " +
                                        "Use development package for shared objects (requires transport). " +
                                        "Examples: '$TMP', 'ZPACKAGE', 'ZFI'", required = true) String packageName,
        ...
        /**
         * MCP Tool: Create a new ABAP program.
         *
        ...
        public CreationResult create_program(
                        @McpToolParam(description = "Name of the program. " +
                                        "Must start with letter, max 30 chars, only A-Z0-9_. " +
                                        "Convention: Start with ZREP_ or Z_ for custom programs. " +
                                        "Examples: 'ZREP_TEST', 'Z_MY_PROGRAM'", required = true) String programName,
                        @McpToolParam(description = "Description of the program (max 60 chars). " +
                                        "Example: 'Test Program'", required = true) String description,
                        @McpToolParam(description = "Package name. " +
                                        "Use '$TMP' for local objects (no transport). " +
                                        "Use development package for shared objects (requires transport). " +
                                        "Examples: '$TMP', 'ZPACKAGE', 'ZFI'", required = true) String packageName,
                        @McpToolParam(description = "Optional transport number. " +
                                        ...
                                        "Leave null for $TMP objects. " +
                                        "Examples: 'CADK911122', 'DEVK900123', null", required = false) String transport) {
                return creationService.createProgram(programName, description, packageName, transport);
        }

        /**
         * MCP Tool: Create a new ABAP include.
         *
        ...
        public CreationResult create_include(
                        @McpToolParam(description = "Name of the include. " +
                                        "Must start with letter, max 30 chars, only A-Z0-9_. " +
                                        "Convention: Ends with _TOP, _F01, etc. " +
                                        "Examples: 'ZREP_TEST_TOP', 'ZREP_TEST_F01'", required = true) String includeName,
                        @McpToolParam(description = "Description of the include (max 60 chars). " +
                                        "Example: 'Top Include'", required = true) String description,
                        @McpToolParam(description = "Package name. " +
                                        "Use '$TMP' for local objects (no transport). " +
                                        "Use development package for shared objects (requires transport). " +
                                        "Examples: '$TMP', 'ZPACKAGE', 'ZFI'", required = true) String packageName,
                        @McpToolParam(description = "Optional transport number. " +
                                        ...
                                        "Leave null for $TMP objects. " +
                                        "Examples: 'CADK911122', 'DEVK900123', null", required = false) String transport) {
                return creationService.createInclude(includeName, description, packageName, transport);
        }
         ...
         *
         * @deprecated Use
         *             {@link com.crystal.mcp.sapserver.tool.DeletionTools#delete_object(String, String, String, String)}
         */
        /*
         * @McpTool(
