 ...
 *
 * This component provides tools for working with ABAP programs (reports, module
 * pools, etc.).
 ...
 * Stage 2: get_object_structure (ObjectTools) → Get program metadata, includes
 * list
 ...
 * - modify_program_source: Modify program/include with workflow (LOCK → MODIFY
 * → UNLOCK)
 *
 * Future Tools:
...
public class ProgramTools {
         ...
         */
        @McpTool(description = "Get source code for an ABAP program (report, module pool, etc.). " +
                        ...
                        "WARNING: For programs with multiple includes, this tool implies context loss. " +
        ...
        public ProgramSourceResult get_program_source(
                        @McpToolParam(description = "Name of the ABAP program. " +
                                        "Examples: 'ZREP_INVOICE_LIST' (report), " +
                                        "'SAPMZTEST' (module pool), " +
                                        "'SAPLZFG_UTILS' (function group main program)", required = true) String programName,
         ...
         */
        @McpTool(description = "Get source code for a program include. " +
        ...
        public IncludeSourceResult get_include_source(
                        @McpToolParam(description = "Name of the parent ABAP program that contains the include. " +
                                        "Examples: 'ZREP_INVOICE_LIST', 'SAPMZTEST', 'ZCL_TEST'", required = true) String programName,
                        @McpToolParam(description = "Name of the include to retrieve. " +
                                        "Examples: 'ZREP_TOP' (top include), " +
                                        "'ZREP_F01' (form include), " +
                                        "'ZCL_TEST===============CCAU' (class auxiliary include)", required = true) String includeName,
                        @McpToolParam(description = "Version to retrieve: 'active' for activated code or 'inactive' for draft. "
                                        +
                                        "Default: 'active'", required = false) String version) {
         ...
         *
         * Error Handling:
         * - If object is locked by another user: Returns error with lock owner
         * information
         ...
         */
        @McpTool(description = "Modify ABAP program or include source code with complete workflow (LOCK → MODIFY → UNLOCK). "
                        +
        ...
        public ProgramModifyResult modify_program_source(
                        @McpToolParam(description = "Name of the program or include to modify. " +
                                        "Examples: 'ZREP_INVOICE_LIST' (program), " +
                                        "'ZFIAAC002' (program), " +
                                        "'ZFIAAC002V_1' (include), " +
                                        "'ZREP_TOP' (include)", required = true) String objectName,
                        @McpToolParam(description = "New source code to set (complete replacement). " +
                                        "Must be valid ABAP syntax. " +
                                        "For programs: Include REPORT/PROGRAM statement. " +
                                        "For includes: No REPORT statement needed.", required = true) String newSource,
                        @McpToolParam(description = "Type of object to modify: 'program' or 'include'. " +
                                        "'program': Reports, module pools, function group mains. " +
                                        "'include': Top includes, form includes, class includes.", required = true) String objectType,
                        @McpToolParam(description = "Optional transport number. " +
                                        ...
                                        "Leave null to use automatic transport assignment.", required = false) String transport) {
         ...
         * Function modules have a different URI structure and require both module and
         * group names.
         * <p>
         * Workflow Steps:
         ...
         *                           system-assigned transport from LOCK)
         * @return ProgramModifyResult with detailed workflow execution status
         */
        @McpTool(description = "Modify ABAP function module source code with complete workflow (LOCK → MODIFY → UNLOCK). "
                        +
                        "Workflow-based tool that handles locking, modification, and unlocking automatically. " +
        ...
        public ProgramModifyResult modify_function_module(
                        @McpToolParam(description = "Name of the function module to modify. " +
                                        "Examples: 'Z_TEST_FM', 'Z_GET_INVOICE', 'Z_PROCESS_ORDER'", required = true) String functionModuleName,
                        @McpToolParam(description = "Parent function group name. " +
                        ...
                        @McpToolParam(description = "New source code to set (complete replacement). " +
                                        ...
                                        +
                                        "Signatures must be configured separately in SE37.", required = true) String newSource,
                        @McpToolParam(description = "Optional transport number. " +
                                        "If not provided, uses system-assigned transport from LOCK operation. " +
                                        "Example: 'CADK911122', 'DEVK900123'. " +
                                        "Leave null to use automatic transport assignment.", required = false) String transport) {
                return programService.modifyFunctionModuleSource(functionModuleName, functionGroupName, newSource,
                                transport);
        }
}
