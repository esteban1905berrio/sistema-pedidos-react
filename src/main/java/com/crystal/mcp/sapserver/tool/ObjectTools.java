...
public class ObjectTools {
     ...
     *
     * This tool implements Progressive Discovery Stage 2: Get metadata before fetching source.
     ...
     *
     * @param objectUri ADT URI for the object (from search results, without /objectstructure)
     * @return ObjectStructure containing metadata and components
     */
    @McpTool(
            description = "Get structure/metadata for any ABAP object (class, program, function group, etc.) " +
    ...
    public ObjectStructure get_object_structure(
            @McpToolParam(
                    description = "ADT URI of the object (without /objectstructure suffix). " +
                            "Obtain from search_objects results. " +
                            "Format examples: " +
                            "'/sap/bc/adt/oo/classes/{name}' (class), " +
                            "'/sap/bc/adt/programs/programs/{name}' (program), " +
                            "'/sap/bc/adt/functions/groups/{name}' (function group)",
                    required = true
            )
            String objectUri
    ) {
     ...
     * Only use after search_objects identified the object and you need the actual code.
     *
     * Token Optimization:
     ...
     */
    @McpTool(
            description = "Get source code for any ABAP object (class, program, function group, etc.) by URI. " +
